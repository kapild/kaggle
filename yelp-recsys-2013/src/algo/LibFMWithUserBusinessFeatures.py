...
class LibFMWithUserBusinessFeatures(BaseModel):
	...
	def get_feature_list(self):
		...
		feat_head = [
			...
			['business_cat1,business_cat2,business_cat3', self.get_category_feat_line, True],
			...
			['user_cool', self.get_user_cool, True],
			['user_funny', self.get_user_funny, True],
			['user_useful', self.get_user_useful, True],
...
#			['city_text', self.get_bus_city_text, True],
	...
	def get_category_num_feat_line(self, bus, user, review_item, bus_test, user_test):

		bus_first_cat = self.get_category_feat(bus_test, user_test, review_item)

		return str(len(bus_first_cat))
